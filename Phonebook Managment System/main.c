...
void namefun() {
	...
	if(fptr==NULL)
	{
		...
		gotoxy(52,10);
		scanf("%lf",&phone);
		fprintf(fptr,"%s %s %s %s %.0lf\n",name,phone,address,gender,gmail);
	}
	fclose(fptr);
	system("cls");
	char ch;
	gotoxy(31,4);
	printf("Do you wanna add other persons record? Press y for that:");
	Sleep(1000);
	fflush(stdin);
	while((ch=getch())=='y'){
		menu();
...
void exitfun()
{
	...
	gotoxy(31,6);
	printf("\xDB Vishesh jat");
	gotoxy(31,8);
	printf("\xDB Sneha patel");
	gotoxy(31,10);
	printf("\xDB Rahul vaghela");
	gotoxy(31,12);
	printf("\xDB Ajay prajapati");
}
void password(void){
	char passwords[20]={"whitehatcode"};
	gotoxy(22,2);
	int j;
	int z;
